...
# Нумерация пунктов вида «п. 1,5» выглядит как дробное число, но денежной
# записью не является: целая часть короче трёх цифр.
w = validate_text("п. 1,5\n(далее — Договор)")
check("номер пункта с дробной частью расхождением не становится",
      not any(t == "amount_mismatch" for t, _ in w), repr(w))

# Тот же фильтр применяется к поиску суммы: стоящее в конце строки число
# пункта не связывается с валютой из начала следующей строки.
f = extract_fields("п. 1,5\nтг перечисляются на счёт")
check("номер пункта и валюта с новой строки не образуют дробную сумму",
      f["amounts"] == [], repr(f["amounts"]))

# В пределах одной строки валюта подтверждает денежный смысл короткой дроби.
f = extract_fields("Комиссия 1,5 тг")
check("короткая дробная сумма в одной строке сохраняется",
      [a["value"] for a in f["amounts"]] == ["1"], repr(f["amounts"]))
