    ...
    while True:
        ...
        try:
            ...
            if attempts > MAX_DOCUMENT_POOL_RETRIES:
                ...
                # Бюджет пересозданий при этом НЕ возвращается: иначе
                # глобальный предохранитель никогда не сработал бы, и при
                # системном сбое сервис вечно числился бы готовым.
                concurrency.discard_pool(generation)
                raise
            log.warning(
