...
def discard_pool(seen_generation: int) -> None:
    """Снять сломанный экземпляр пула, не трогая бюджет восстановления.

    Вызывается, когда обработку прекращает сам документ: пул ломается именно
    на нём, локальная граница повторов исчерпана, пробовать дальше
    бессмысленно.

    Смысл ровно один — не оставлять сломанный экземпляр установленным.
    Иначе следующий документ получит его же и упадёт, не начав работу.

    Бюджет пересозданий сознательно НЕ возвращается. Возврат превратил бы
    глобальный предохранитель в мёртвый механизм: каждый документ
    заканчивался бы либо успехом (бюджет сброшен), либо сдачей (бюджет
    возвращён), счётчик никогда не дорос бы до предела, и при системном
    сбое сервис вечно отвечал бы `ready`, падая на каждом PDF.

    Поэтому отравленный документ оставляет следы своих пересозданий. Если
    следующий документ пройдёт, `note_pool_success` их сбросит; если падения
    продолжатся, бюджет честно исчерпается и `/readyz` отдаст 503.
    """
