...
PDF-сканов (RU/KK). Сборка воспроизводима: базовый образ закреплён по
digest, Python-зависимости — по lock-файлу, языковые модели — по sha256.
...
- Paddle-таргет: `requirements-paddle.txt` (`paddlepaddle==3.0.0`,
  `paddleocr==3.0.0`). Транзитивный lock для full не ведём: таргет
  собирается только на amd64, а рабочая машина разработки — arm64.
- Системные пакеты Debian (tesseract-ocr 5.3.0, poppler-utils и пр.)
  фиксируются digest'ом базового образа и репозиторием bookworm на момент
  сборки; точный список — `docker run --rm <образ> dpkg -l`.
